...
function SignZedAndItsFriends {
    ...
    $files = "$innoDir\LingCode.exe,$innoDir\cli.exe,$innoDir\auto_update_helper.exe,$innoDir\zed_explorer_command_injector.dll,$innoDir\zed_explorer_command_injector.appx"
...
function BuildInstaller {
    ...
    switch ($channel) {
        "stable" {
            ...
            $appName = "LingCode"
            $appDisplayName = "LingCode"
            $appSetupName = "LingCode-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "LingCode-Editor-Stable-Instance-Mutex"
            $appExeName = "LingCode"
            $regValueName = "LingCode"
            $appUserId = "LingCode.LingCode"
            $appShellNameShort = "Lin&gCode"
            $appAppxFullName = "ZedIndustries.Zed_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "preview" {
            $appId = "{{F70E4811-D0E2-4D88-AC99-D63752799F95}"
            $appIconName = "app-icon-preview"
            $appName = "LingCode Preview"
            $appDisplayName = "LingCode Preview"
            $appSetupName = "LingCode-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "LingCode-Editor-Preview-Instance-Mutex"
            $appExeName = "LingCode"
            $regValueName = "LingCodePreview"
            $appUserId = "LingCode.LingCode.Preview"
            $appShellNameShort = "Lin&gCode Preview"
            $appAppxFullName = "ZedIndustries.Zed.Preview_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "nightly" {
            $appId = "{{1BDB21D3-14E7-433C-843C-9C97382B2FE0}"
            $appIconName = "app-icon-nightly"
            $appName = "LingCode Nightly"
            $appDisplayName = "LingCode Nightly"
            $appSetupName = "LingCode-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "LingCode-Editor-Nightly-Instance-Mutex"
            $appExeName = "LingCode"
            $regValueName = "LingCodeNightly"
            $appUserId = "LingCode.LingCode.Nightly"
            $appShellNameShort = "Lin&gCode Nightly"
            $appAppxFullName = "ZedIndustries.Zed.Nightly_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "dev" {
            $appId = "{{8357632E-24A4-4F32-BA97-E575B4D1FE5D}"
            $appIconName = "app-icon-dev"
            $appName = "LingCode Dev"
            $appDisplayName = "LingCode Dev"
            $appSetupName = "LingCode-$Architecture"
            # The mutex name here should match the mutex name in crates\zed\src\zed\windows_only_instance.rs
            $appMutex = "LingCode-Editor-Dev-Instance-Mutex"
            $appExeName = "LingCode"
            $regValueName = "LingCodeDev"
            $appUserId = "LingCode.LingCode.Dev"
            $appShellNameShort = "Lin&gCode Dev"
