...
const URL_PREFIX: [&'static str; 5] = ["lingcode://", "http://", "https://", "file://", "ssh://"];
...
fn main() -> Result<()> {
    ...
    let url = format!("lingcode-cli://{server_name}");
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://lingcode.dev/docs/remote-development"
    );
...
mod flatpak {
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("dev.lingcode.LingCode"))
            && args.zed.is_none()
        {
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.lingcode.LingCode") {
                return None;
            }
