...
impl OpenRequest {
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("lingcode-cli://") {
            ...
            } else if let Some(file) = url.strip_prefix("lingcode://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("lingcode://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("lingcode://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if let Some(session_id_str) = url.strip_prefix("lingcode://agent/shared/") {
            ...
            } else if let Some(agent_path) = url.strip_prefix("lingcode://agent") {
                this.parse_agent_url(agent_path)
            } else if let Some(schema_path) = url.strip_prefix("lingcode://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "lingcode://settings" || url == "lingcode://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("lingcode://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("lingcode://git/clone") {
...
mod tests {
    ...
    fn agent_url_with_prompt(prompt: &str) -> String {
        let mut serializer = url::form_urlencoded::Serializer::new("lingcode://agent?".to_string());
    ...
    fn test_parse_shared_agent_thread_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![format!("lingcode://agent/shared/{session_id}")],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_shared_agent_thread_url_with_invalid_uuid(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["lingcode://agent/shared/not-a-uuid".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["lingcode://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["lingcode://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["lingcode://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["lingcode://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "lingcode://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fzed-industries%2Fzed.git"
                            .into(),
                    ],
                    ..Default::default()
                },
