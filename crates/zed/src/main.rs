...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "LingCode failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.lingcode.Oops";
            proxy
                .add_notification(
                    notification_id,
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("LingCode System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    if let Some(kind) = request.kind {
        match kind {
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("lingcode://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // lingcode://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // lingcode://settings/languages/Rust/tab_size  - SUPPORT
...
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `lingcode://` scheme, or relative to <https://zed.dev>.
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("lingcode://")
                || arg.starts_with("lingcode-cli://")
                || arg.starts_with("ssh://")
